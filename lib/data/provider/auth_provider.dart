...
class AuthApiClient{
  ...
  Future<Map<String, dynamic>> login(String username, String password) async {
    try{
      ...
      var response = await http.post(Uri.parse(baseUrlLogin), body: {
        "username": username,
        "password": password
      });
      if(response.statusCode == 200){
        ...
        Get.defaultDialog(
          title: "Login",
          content: Text("${jsonDecode(response.body)['error']} : Usuário Inválido")
        );
  ...
  Future<Map<String, dynamic>> register(String username, String password) async {
    try{
      ...
      var response = await http.post(Uri.parse(baseUrlRegister), body: {
        "username": username,
        "password": password
      });
      if(response.statusCode == 200){
